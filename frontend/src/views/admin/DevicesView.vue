  ...
  <div>
    ...
    <el-radio-group v-model="selectedId" style="width: 100%">
      ...
      >
        ...
        <el-table-column prop="leadTime" label="交货周期" />
...
async function load() {
  try {
    list.value = await unwrap(http.get('/api/admin/crm/devices'))
...
async function removeSelected() {
  ...
  try {
    ...
    await unwrap(http.post(`/api/admin/crm/devices/${selectedId.value}/delete`))
...
async function onExcelFile(e: Event) {
  ...
  try {
    ...
    const count = await unwrap<number>(
      http.post('/api/admin/crm/devices/import', fd, { headers: { 'Content-Type': 'multipart/form-data' } })
    )
