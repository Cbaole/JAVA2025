...
async function load() {
  if (!id || id === 'new') return
  try {
    const data: any = await unwrap(http.get(`/api/admin/crm/devices/${id}`))
...
async function save() {
  ...
  try {
    ...
    const newId = await unwrap<string>(http.post('/api/admin/crm/devices/upsert', payload))
    ElMessage.success('保存成功')
    if (!id || id === 'new') {
      router.replace(`/admin/devices/${newId}`)
      return
    }
    await load()
  } catch (e: any) {
...
async function onFiles(e: Event) {
  ...
  try {
    for (const file of files) {
      ...
      fd.append('file', file)
      await unwrap(
        http.post(`/api/admin/crm/devices/${id}/attachments/upload`, fd, { headers: { 'Content-Type': 'multipart/form-data' } })
      )
    }
...
async function download(row: any) {
  if (!row?.id) return
  try {
    const resp = await http.get(`/api/admin/crm/devices/attachments/${row.id}/download`, { responseType: 'blob' })
...
async function removeAttachment(row: any) {
  ...
  try {
    ...
    await unwrap(http.post(`/api/admin/crm/devices/attachments/${row.id}/delete`))
