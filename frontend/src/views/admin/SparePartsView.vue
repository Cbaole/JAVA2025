...
async function load() {
  try {
    list.value = await unwrap(http.get('/api/admin/crm/spare-parts'))
...
function add() {
  router.push('/admin/spare-parts/new')
}

function edit(id: string) {
  router.push(`/admin/spare-parts/${id}`)
}
...
async function removeSelected() {
  if (!selectedId.value) return
  try {
    await ElMessageBox.confirm('确认删除该备品备件？', '提示', { type: 'warning' })
    await unwrap(http.post(`/api/admin/crm/spare-parts/${selectedId.value}/delete`))
...
async function onExcelFile(e: Event) {
  ...
  try {
    ...
    const count = await unwrap<number>(
      http.post('/api/admin/crm/spare-parts/import', fd, { headers: { 'Content-Type': 'multipart/form-data' } })
    )
