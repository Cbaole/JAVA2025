...
import com.example.perm.repo.DeviceAttachmentRepository;
...
@RestController
@RequestMapping("/api/admin/crm/devices")
public class AdminDeviceController {
    private final DeviceRepository deviceRepository;
    private final DeviceAttachmentRepository attachmentRepository;

    public AdminDeviceController(DeviceRepository deviceRepository, DeviceAttachmentRepository attachmentRepository) {
        this.deviceRepository = deviceRepository;
        this.attachmentRepository = attachmentRepository;
    }

    @GetMapping
    @PreAuthorize("@perm.has(authentication,'crm/device','see')")
    public ApiResponse<Object> list() {
        return ApiResponse.ok(deviceRepository.findAll());
    }

    @GetMapping("/{id}")
    @PreAuthorize("@perm.has(authentication,'crm/device','read')")
    public ApiResponse<Object> detail(@PathVariable String id) {
        var dev = deviceRepository.findById(id).orElseThrow(() -> new IllegalArgumentException("设备不存在"));
        var atts = attachmentRepository.findByDeviceOrderByCreateTimeDesc(dev);
        return ApiResponse.ok(new Object() {
            public final DeviceEntity base = dev;
            public final java.util.List<DeviceAttachmentEntity> attachments = atts;
        });
    }

    @PostMapping("/upsert")
    @Transactional
    @PreAuthorize("@perm.has(authentication,'crm/device','add') or @perm.has(authentication,'crm/device','update')")
    public ApiResponse<Object> upsert(@RequestBody Map<String, Object> req) {
        DeviceEntity entity = (req.get("id") == null || ((String) req.get("id")).isBlank())
                ? new DeviceEntity()
                : deviceRepository.findById((String) req.get("id")).orElseThrow(() -> new IllegalArgumentException("设备不存在"));
        ...
        try (var wb = WorkbookFactory.create(file.getInputStream())) {
            ...
            if (headerRow != null) {
                for (Cell cell : headerRow) {
                    ...
                    var key = deviceHeaderKey(header);
    ...
    @Transactional
    @PreAuthorize("@perm.has(authentication,'crm/device','update')")
    public ApiResponse<Object> upload(@PathVariable String id, @RequestParam("file") MultipartFile file) throws Exception {
        ...
        var att = new DeviceAttachmentEntity();
        att.setDevice(dev);
    ...
    private static String deviceHeaderKey(String header) {
